...
TEMPLATE_SOIL_PATH = "{local_path_to_data_dir}germany/SAARbuek200_100_25832_etrs89-utm32n.asc"  # Lower Saxony 100 m
        ...
        if orig_spec.startswith("{"):
            ...
            if "start" in stage_spec and "end" in stage_spec:
                ...
                for vals in results:
                    ...
                    if vals["Stage"] == stage:
                        stage_threshold = TRADEF_THRESHOLDS.get(vals["Stage"])
                        # if vals["TraDef"] < 1:
                        if vals["TraDef"] < stage_threshold:
                            days_below_tradef[cm_count] += 1
                        days_in_stage[cm_count] += 1
                        if not year_crop_set:
                            cm_count_to_data[cm_count] = (vals["Year"], vals["Crop"])
                            year_crop_set = True
                for cm_count, (year, crop) in cm_count_to_data.items():
                    cm_count_to_vals[cm_count][f"AvgDaysTraDefBelowDST_Stage{stage}"] = days_below_tradef[cm_count] / days_in_stage[cm_count]
...
def write_row_to_grids(row_col_data, row, ncols, header, path_to_output_dir, path_to_csv_output_dir, setup_id):
    ...
    output_grids = {
        ...
        "AvgDaysTraDefBelowDST_Stage7": {"data": make_dict_nparr(), "cast-to": "float", "digits": 2}
    }
