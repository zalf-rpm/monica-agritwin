...
def run_producer(server={"server": None, "port": None}, shared_id=None):
    ...
    config = {
        ...
        "end-row": "-1",
        "path_to_dem_grid": "",
        "sim.json": "sim_avgyield.json",
        "crop.json": "crop2.json",
        "site.json": "site.json",
        "setups-file": "sim_setups_state_proj.csv",
        "run-setups": "[19]",
        "shared_id": shared_id
    }
    ...
    def apply_regional_crop_params(env, crop_code, nuts3_id):
        if not nuts3_id:
            return None

        crop_lookup = crop_param_tables.get(crop_code)

        if not crop_lookup:
            return None

        region_params = crop_lookup.get(str(nuts3_id).strip())
    ...
    for _, setup_id in enumerate(run_setups):
        ...
        start_year = int(str(setup["start_date"])[:4])
        end_year = int(str(setup["end_date"])[:4])

        simulation_years = tuple(range(start_year, end_year + 1))

        # Store the union of all yearly crop maps as a compact bit mask. The simulation remains continuous, while the
        # consumer can select crop cells separately for each output year.
        crop_year_mask = np.zeros(soil_grid.shape, dtype=np.uint64)
        crop_map_dir = os.path.join(paths["path-to-projects-dir"], DATA_GRID_CROPS_DIR)
        for year_index, crop_year in enumerate(simulation_years):
            crop_map_filename = crop_data.format(year=crop_year)
            path_to_crop_grid = os.path.join(crop_map_dir, crop_map_filename)
            if not os.path.isfile(path_to_crop_grid):
                raise FileNotFoundError(f"Missing crop map for setup {setup_id}, year {crop_year}: {path_to_crop_grid}")

            crop_meta, _ = Mrunlib.read_header(path_to_crop_grid)
            crop_grid = np.loadtxt(path_to_crop_grid, dtype=np.int16, skiprows=6)
            crop_year_mask[crop_grid == 1] |= np.uint64(1 << year_index)
            # print(f"read crop map for {crop_year}: {path_to_crop_grid}")
        ...
        if setup["irrigation"] and setup.get("irrigation_data"):
        ...
        for srow in range(0, srows):
            ...
            for scol in range(0, scols):
                ...
                cell_crop_year_mask = int(crop_year_mask[srow, scol])
                if cell_crop_year_mask == 0 or soil_id == -8888:
                ...
                crop_code = crop_id_short
                apply_regional_crop_params(env, crop_code, region_name)
                ...
                if setup["irrigation"] and irrigation == 1:
                    ...
                    if irrigation_manager.should_be_irrigated_by_crop_id(setup["crop-id"]):
                        env["params"]["simulationParameters"]["UseAutomaticIrrigation"] = True
                        # add default values for irrigation amount and threshold
                        env["params"]["simulationParameters"]["AutoIrrigationParams"]["amount"] = [10, "mm"]
                        env["params"]["simulationParameters"]["AutoIrrigationParams"][
                            "trigger_if_nFC_below_%"] = [30, "%"]
                        env["params"]["simulationParameters"]["AutoIrrigationParams"]["set_to_%nFC"] = [100, "%"]
                        env["params"]["simulationParameters"]["AutoIrrigationParams"][
                            "calc_nFC_until_depth_m"] = [0.3, "m"]
                        # print("irrigation amount:",
                        #       env["params"]["simulationParameters"]["AutoIrrigationParams"]["amount"])
                    else:
                        env["params"]["simulationParameters"]["UseAutomaticIrrigation"] = False
                        # reset irrigation amount and threshold
                        env["params"]["simulationParameters"]["AutoIrrigationParams"]["amount"] = [0, "mm"]
                        env["params"]["simulationParameters"]["AutoIrrigationParams"][
                    ...
                    env["params"]["simulationParameters"]["UseAutomaticIrrigation"] = False
                    env["params"]["simulationParameters"]["AutoIrrigationParams"]["amount"] = [0, "mm"]
                    env["params"]["simulationParameters"]["AutoIrrigationParams"]["trigger_if_nFC_below_%"] = [50, "%"]
                    env["params"]["simulationParameters"]["AutoIrrigationParams"]["set_to_%nFC"] = [100, "%"]
                    env["params"]["simulationParameters"]["AutoIrrigationParams"]["calc_nFC_until_depth_m"] = [0.5, "m"]
                ...
                env["customId"] = {
                    ...
                    "env_id": sent_env_count,
                    "nodata": False,
                    "crop_years": [crop_year for year_index, crop_year in enumerate(simulation_years)
                                   if cell_crop_year_mask & (1 << year_index)]
                }
