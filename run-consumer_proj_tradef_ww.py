...
TEMPLATE_SOIL_PATH = "{local_path_to_data_dir}germany/SAARbuek200_100_25832_etrs89-utm32n.asc"
...
def create_output(msg, crop_years=None):
    allowed_crop_years = None if crop_years is None else {int(year) for year in crop_years}
    cm_count_to_vals = defaultdict(dict)
    for data in msg.get("data", []):
        results = data.get("results", [])

        orig_spec = data.get("origSpec", "")
        is_daily_section = orig_spec == '"daily"'
        if orig_spec.startswith("{"):
            stage_spec = json.loads(orig_spec)
            if "start" in stage_spec and "end" in stage_spec:
                stage = int(stage_spec["start"][-1])
                days_in_stage = defaultdict(lambda: 0)
                days_below_tradef = defaultdict(lambda: 0)
                cm_count_to_data = {}

                for vals in results:
                    cm_count = vals["CM-count"]
                    if vals["Stage"] == stage:
                        stage_threshold = TRADEF_THRESHOLDS[stage]
                        if vals["TraDef"] < stage_threshold:
                            days_below_tradef[cm_count] += 1
                        days_in_stage[cm_count] += 1
                        cm_count_to_data.setdefault(cm_count, (vals["Year"], vals["Crop"]))
                for cm_count, (year, crop) in cm_count_to_data.items():
                    cm_count_to_vals[cm_count][f"AvgDaysTraDefBelowDST_Stage{stage}"] = days_below_tradef[cm_count] / \
                                                                                        days_in_stage[cm_count]
                    cm_count_to_vals[cm_count]["Crop"] = crop
                    cm_count_to_vals[cm_count]["Year"] = year
        else:
            for vals in results:
                if "CM-count" in vals:
    ...
    if allowed_crop_years is not None:
        for vals in cm_count_to_vals.values():
            try:
                result_year = int(vals["Year"])
            except (KeyError, TypeError, ValueError):
                continue
            vals["crop-map-active"] = result_year in allowed_crop_years
...
def write_row_to_grids(row_col_data, row, ncols, header, path_to_output_dir, path_to_csv_output_dir, setup_id):
    ...
    output_grids = {
        "AvgDaysTraDefBelowDST_Stage1": {"data": make_dict_nparr(), "cast-to": "float", "digits": 2},
        "AvgDaysTraDefBelowDST_Stage2": {"data": make_dict_nparr(), "cast-to": "float", "digits": 2},
        "AvgDaysTraDefBelowDST_Stage3": {"data": make_dict_nparr(), "cast-to": "float", "digits": 2},
        "AvgDaysTraDefBelowDST_Stage4": {"data": make_dict_nparr(), "cast-to": "float", "digits": 2},
        "AvgDaysTraDefBelowDST_Stage5": {"data": make_dict_nparr(), "cast-to": "float", "digits": 2},
        "AvgDaysTraDefBelowDST_Stage6": {"data": make_dict_nparr(), "cast-to": "float", "digits": 2},
    }
                    ...
                    for cell_data in rcd_val:
                        ...
                        for cm_count, data in cell_data.items():
                            for key in output_keys:
                                ...
                                if key in data:
                                    v = data[key] if data.get("crop-map-active", True) else -9999
                                    if key == "Yield" and v != -9999:
    ...
    def process_message(msg):
        ...
        if not write_normal_output_files:
            ...
            if is_nodata:
                ...
                data["row-col-data"][row][col].append(create_output(msg, custom_id.get("crop_years")))
            ...
            path_to_out_dir = config["csv"] + str(setup_id) + "/" + str(row) + "/"
            print(path_to_out_dir)
