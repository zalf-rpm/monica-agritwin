...
- **`run-producer_nuts3_sensitivity_fcm.py`**: Producer script for sensitivity analysis at NUTS3 level with soiltype and NUTS3-specific field condition modifiers
...
- **`run-producer_nuts3_avgyield.py`**: Producer script for NUTS3 average yield simulations
- **`run-producer_nuts3_avgyield_fcm.py`**: Producer script for NUTS3 average yield simulations with NUTS3-specific field condition modifiers
- **`run-consumer_nuts3_avgyield.py`**: Consumer script to aggregate NUTS3 average yield results into CSV format. The CSV file includes columns for the year, average yield in t/ha DM, NUTS3 region name, and soil type.
